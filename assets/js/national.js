document.addEventListener('DOMContentLoaded', function() {
    console.log("national.js loaded - starting execution");
    ...
    try {
        ...
        async function loadAllArticles() {
            ...
            try {
                ...
                // Filter client-side
                allNationalArticles = querySnapshot.docs
                    .filter(doc => doc.data().category === 'National News')
                    .map(doc => ({
                        id: doc.id,
                        ...doc.data()
                    }));

                console.log(`Found ${allNationalArticles.length} national articles`);
                
                if (allNationalArticles.length === 0) {
        ...
        function renderPage() {
            ...
            const pageArticles = allNationalArticles.slice(startIdx, endIdx);
        ...
        function showLoadingState() {
            elements.container.innerHTML = `
                <div class="loading-state">
                    <i class="fas fa-spinner fa-spin"></i> Loading national news...
                </div>
            `;
        }
        ...
        function updatePaginationState() {
            const totalPages = Math.ceil(allNationalArticles.length / articlesPerPage);
        ...
        function handleEmptyResults(page) {
            elements.container.innerHTML = page === 1 
                ? '<p>No national news articles found.</p>' 
                : '<p>No more national news articles to show.</p>';
            hasMore = false;
        }
